...
```
flutter_otel_api ──> flutter_otel_exporter_otlp_http ──┬──> flutter_otel_sdk
                                                        └──> flutter_otel (umbrella)
```

Depends on `flutter_otel_api` and `http`. Depended on by `flutter_otel_sdk`
...
```bash
cd packages/flutter_otel_exporter_otlp_http
