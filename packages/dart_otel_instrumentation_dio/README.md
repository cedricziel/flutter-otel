...
This is the first of the `dart_otel_instrumentation_<target>` family —
...
```
dart_otel_api ──> dart_otel_instrumentation_dio
```

Depends on `dart_otel_api` and `dio`. Nothing else in the workspace
...
dio.interceptors.add(
  DioOTelInterceptor.privacy(
    ...
    tracer: OTelSdk.instance.getTracer(name: 'dart_otel_instrumentation_dio'),
  ),
);
```
...
dependencies:
  ...
  dart_otel_instrumentation_dio:
    git:
      url: https://github.com/cedricziel/flutter-otel
      ref: main
      path: packages/dart_otel_instrumentation_dio
...
```bash
cd packages/dart_otel_instrumentation_dio
