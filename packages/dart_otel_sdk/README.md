...
interfaces declared in [`dart_otel_api`](../dart_otel_api/README.md)
...
```
dart_otel_api  ──┐
                     ├──> dart_otel_sdk ──> flutter_otel (umbrella)
dart_otel_exporter_otlp_http ──┘
```

Depends on `dart_otel_api`, `dart_otel_exporter_otlp_http`, `flutter`
...
Future<void> main() async {
  final sdk = await OTelSdk.initialize(
    OTelSdkConfig(
      ...
      // Bring your own exporters, e.g. dart_otel_exporter_otlp_http's
  ...
  await sdk.getTracer().startActiveSpan('startup', (span) async {
    sdk.getLogger().info('hello from dart_otel_sdk');
  });
  await sdk.shutdown();
}
```
