...
- Wire format (finalized here; every task must match these field names exactly):
  ...
  - `?`-suffixed fields (`parentSpanId`, `statusDescription`, `scopeVersion`, log `traceId`/`spanId`) are truly optional: Task 8's Swift producer omits the JSON key entirely rather than writing `null`, and Task 3's Dart decoder treats a missing key as `null`.
  - The remaining fields (`attributes`, `events`, `statusCode`, `scopeName` on spans; `attributes` on logs) are always present on the wire — Task 8's producer emits them unconditionally, using `{}`/`[]`/`"flutter_otel_native"` for the "nothing to report" case rather than omitting the key. Task 3's decoder is intentionally more lenient than this guarantee requires — it defaults `attributes`/`events`/`scopeName` when a line happens to omit them anyway, rather than rejecting an otherwise-well-formed record over a cosmetic empty-collection field — which is a deliberate robustness choice, not a contract violation: nothing produced by this plan ever exercises that fallback, but a future producer variant that drops an empty `{}`/`[]` to save bytes won't lose the whole record. `statusCode`/`spanKind` remain strictly required (decode failure returns `null`) since they carry real information that can't be defaulted meaningfully.
